    ...
    def screenshot(self):
        ...
        def cord_info(x,y):
            with mss() as sct:
                ...
                real_x = y
                real_y = y
                clamped_x = restrict(monitor['left']+middle,x,monitor['width']-middle-1)
                clamped_y = restrict(monitor['top']+middle,y,monitor['height']-middle-1)
                # returns the actual cordinates and then cords that dont go outside of screen 
                return(x,y,clamped_x,clamped_y)
        ...
        x, y, clamped_x, clamped_y, = cord_info(x,y)
        diff_x = x - clamped_x
        diff_y = y - clamped_y
        #print(diff_x,diff_y)
        ...
        # apply a 10px border aropund whole image
...
class Main:
    def __init__(self):
        ...
        # color value buttons
        self.root.label_color_rgb = tk.Button(self.root,text = "128128128",borderwidth=0)
    ...
    def copy_color_value(self,val):
        def decode(val):
            if val == 'rgb': return self.selected_color_rgb
            if val == 'hex': return self.selected_color_hex
    ...
    def update(self):
        ...
        #min_val =  get_min(self.root.winfo_width(),self.root.winfo_height()-100)
        #self.preview_size = min_val
